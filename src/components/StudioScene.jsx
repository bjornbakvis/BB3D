...
function Room({ roomW, roomD, wallH, showWalls, wallMap, wallOpacity, cameraAction }) {
  ...
  // Freeze hidden-wall updates briefly during camera reset to avoid visual flicker
  // while OrbitControls damping/velocity settles.
  const freezeUntilRef = useRef(0);
  const lastResetNonceRef = useRef(null);

  useEffect(() => {
    if (!cameraAction || cameraAction.type !== "reset" || !cameraAction.nonce) return;
    if (lastResetNonceRef.current === cameraAction.nonce) return;
    lastResetNonceRef.current = cameraAction.nonce;

    const now = typeof performance !== "undefined" && performance.now ? performance.now() : Date.now();
    freezeUntilRef.current = now + 220; // ms
  }, [cameraAction]);
...
<directionalLight
        ...
        {/* Room walls */}
        <Room roomW={roomW} roomD={roomD} wallH={wallH} showWalls={showWalls} wallMap={wallTex} wallOpacity={theme.wall.opacity} cameraAction={cameraAction} />
