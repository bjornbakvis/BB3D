  ...
  function constrainAndMagnet({ id, x, y, z, w, d, h, rotY, objectsNow }) {
    ...
    if (movingType === "Countertop") {
      ...
      if (cabinets.length >= 2) {
        const tolLane = 0.10;   // how strict they must be aligned on Z
        const tolAdj = 0.08;    // how strict they must be "next to each other"
        const tolWidth = 0.06;  // width match tolerance (meters)
        const tolNear = 0.25;   // how close countertop must be to trigger snap

        // For now: only support rotY ~ 0 to avoid confusing snaps for rotated slabs.
        if (Math.abs((rotY || 0) % (Math.PI * 2)) < 1e-3) {
